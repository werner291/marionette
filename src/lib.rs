...
mod tests {
    use super::*;
    use nalgebra::{Vector2, Isometry2, Isometry3, Vector3, Point2, Rotation2};
    ...
    use crate::path::{LerpPathDiscretePath, ParametrizedPath, parametrize_by_distance, OutOfRangeError, CompoundPath, build_compound_path_from_steering_function};
    use crate::motion_validation::{UniformLinearInterpolatedSamplingValidator, MotionValidator, StateValidator};
    ...
    fn dubins_parking() {
        ...
        let validate_state = |state:&RigidBodyState2| {
            parked_cars_at.iter().any(|car_pos| {
                ncollide2d::query::proximity(
                    &state.0,
                    &cuboid,
                    &car_pos,
                    &cuboid, 0.0) == ncollide2d::query::Proximity::Intersecting
            }).not()
        };
        ...
        let path = build_compound_path_from_steering_function(path, crate::dubins::compute_dubins_motion, 0.0)
        ...
        let mut agent_car_cube = window.add_cube(cuboid.half_extents.x as f32 * 2.0f32, cuboid.half_extents.x as f32 * 2.0f32, cuboid.half_extents.y as f32 * 2.0f32);

        for pos in &parked_cars_at {
            let mut car_cube = window.add_cube(cuboid.half_extents.x as f32 * 2.0f32, cuboid.half_extents.x as f32 * 2.0f32, cuboid.half_extents.y as f32 * 2.0f32);
    ...
    fn test_integration() {
        ...
        use nalgebra::{Isometry3, Point3, distance};
        ...
        use crate::path::{parametrize_by_distance, TimedState, LerpPathDiscretePath, ParametrizedPath};

        let cuboid = Cuboid::new(Vector3::new(2.0f64, 1.0, 3.0));

        let start_state = PointState(Point3::new(-10.0, 0.0, 0.0));

        let obstacle_xform = Isometry3::new(
        ...
        let validate_state = |state:&PointState| {
            ncollide3d::query::proximity(
                ...
                &cuboid,
                &obstacle_xform,
                &cuboid, 0.0) == ncollide3d::query::Proximity::Intersecting
        };


        let valid = UniformLinearInterpolatedSamplingValidator::new(
            validate_state,
            0.1
        );
