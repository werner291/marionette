use nalgebra::{Point2, Isometry2, Vector2, Rotation2, Translation2};
use crate::path::{ParametrizedPath, OutOfRangeError};
...
impl StraightlineArcPathSegment {
    ...
    fn sample_at(&self, t : f64) -> Isometry2<f64> {
        match self {
            ...
            Self::CircularArc {
                angle, radius, from_point
            } => {
                let mut transform = from_point.clone();
                transform.append_rotation_wrt_point_mut(&Rotation2::new(t/radius * angle.signum()).into(), &(&transform * Point2::new(*radius, 0.0)));
                transform
            }
        }
    }
...
impl ParametrizedPath<RigidBodyState2> for DubinsMotion {
    fn defined_range(&self) -> RangeInclusive<f64> {
        self.start_time ..= self.segments.iter().map(StraightlineArcPathSegment::length).sum::<f64>() + self.start_time
    }
...
pub fn compute_dubins_motion(from: &RigidBodyState2, to: &RigidBodyState2, start_time: f64) -> DubinsMotion {
...
mod tests {
    ...
    use nalgebra::{Isometry2, Vector2, Translation2};
    use rand::prelude::ThreadRng;
    use crate::dubins::compute_dubins_motion;
    use crate::state::rigid_2d::rigid_body_state_2d::RigidBodyState2;
    ...
    fn dubins_validpath() {
        ...
        for _ in 0..100 {
            ...
            for i in 0..=1000 {
                ...
                println!("{}, {}", st1.0.translation.vector.x, st1.0.translation.vector.y);
            }

            assert_eq!(0.0, *motion.defined_range().start());
            let computed_start = motion.sample(*motion.defined_range().start()).expect("Should not give out-of-range.");
            assert!((start_point.0.translation.vector - computed_start.0.translation.vector).norm() < 1.0e-5);
            assert!(start_point.distance(&computed_start) < 1.0e-10);
            let computed_end = motion.sample(*motion.defined_range().end()).expect("Should not give out-of-range.");
            assert!((end_point.0.translation.vector - computed_end.0.translation.vector).norm() < 1.0e-5);
