...
impl<State> TryFrom<DiscretePointsPath<State>> for NonEmptyPointsPath<State> {
    ...
    fn try_from(value: DiscretePointsPath<State>) -> Result<Self, Self::Error> {
        NonEmpty::from_vec(value.states).map( |states| {
            NonEmptyPointsPath {
                states
            }
        }).ok_or(IsEmptyError)
...
pub fn parametrize_by_distance<State:Copy>(path: NonEmptyPointsPath<State>, distance: impl Fn(&State, &State) -> f64, start_point: f64) -> NonEmptyPointsPath<TimedState<State>> {
    NonEmptyPointsPath {
            states:
                NonEmpty {
                    head: TimedState(path.states[0],start_point),
                    tail:  path.states.iter().tuple_windows().scan(start_point, |t, (st1, st2)| {
                        *t += distance(st1, st2);
...
impl<'a, State:Lerp<f64>+Copy> ParametrizedPath<State> for LerpPathDiscretePath<'a, State> {
    ...
    fn sample(&self, t: f64) -> Result<State, OutOfRangeError> {
        if t < self.0.states.first().1 || t >= self.0.states.last().1 { // FIXME range bounds
...
impl<State,F,P> SteeringFunction<State> for F where F : Fn(&State, &State, f64) -> P, P: ParametrizedPath<State> {
    type PathType = P;
...
pub struct NotAtLeastOnePairOfStatesError;
    ...
    pub fn build_compound_path_from_steering_function<Steer,State:Sized>(path:NonEmptyPointsPath<State>,
                                                      steer: Steer,
                                                      start_time: f64) -> Result<CompoundPath<State,Steer::PathType>, NotAtLeastOnePairOfStatesError>
        where Steer: SteeringFunction<State> {

        NonEmpty::from_vec(path.states.iter().tuple_windows().scan(start_time, |t, (st1,st2)| {
                let segment = steer.steer(st1, st2, *t);
                assert_eq!(t, segment.defined_range().start());
                *t = *segment.defined_range().end();
                Some(segment)
            })
            .collect()).map(|segments| CompoundPath { segments, _phantom:PhantomData} )
            .ok_or(NotAtLeastOnePairOfStatesError)
    }

impl<State, Subpath:ParametrizedPath<State>> ParametrizedPath<State> for CompoundPath<State, Subpath> {
    fn defined_range(&self) -> RangeInclusive<f64> {
        *self.segments.first().defined_range().start() ..= *self.segments.last().defined_range().end()
    }

    fn sample(&self, t: f64) -> Result<State, OutOfRangeError> {
        self.segments.binary_search_by(|seg| if t < *seg.defined_range().start() {
            Ordering::Greater
        } else if *seg.defined_range().end() < t {
            Ordering::Less
        } else {
            Ordering::Equal
        }).map_err(|_| OutOfRangeError).and_then(|seg| self.segments[seg].sample(t))
    }
}
...
mod tests {
    ...
    use crate::path::{NonEmptyPointsPath, TimedState, LerpPathDiscretePath, ParametrizedPath};
    use nonempty::NonEmpty;
    ...
    fn test_identity() {
        ...
        let mut states =
            once(-100.0).chain(thread_rng().sample_iter(distr).take(1000)).chain(once(100.0)).map(|t| TimedState(t,t)).collect::<Vec<_>>();
        ...
        for _ in 0 .. 100 {
            ...
            let sample = LerpPathDiscretePath(&path).sample(t).expect("All samples should be in range.");
