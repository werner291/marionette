...
impl<State,F> StateValidator<State> for F where F : Fn(&State) -> bool {
    fn validate_state(&self, st: &State) -> bool {
        self(st)
    }
}
...
pub struct UniformLinearInterpolatedSamplingValidator<State, Validate> where State: Lerp<f64>, Validate: StateValidator<State> {
...
impl<State, Validate> MotionValidator<State> for UniformLinearInterpolatedSamplingValidator<State,Validate>
    where State: Lerp<f64> + Distance<State, DistanceValue=f64>,
          Validate: StateValidator<State> {
...
impl<State,F> MotionValidator<State> for F where F : Fn(&State,&State) -> bool {
    fn validate_motion(&self, st1: &State, st2: &State) -> bool {
        self(st1,st2)
    }
}
...
pub struct SteeredSamplingValidator<State, Validate, Steer> where State: Lerp<f64>, Validate: StateValidator<State>, Steer: SteeringFunction<State> {
    pub validate_state: Validate,
    pub sample_interval: f64,
...
impl<State, Validate, Steer> MotionValidator<State> for SteeredSamplingValidator<State,Validate, Steer>
    where State: Lerp<f64> + Distance<State, DistanceValue=f64>,
          Validate: StateValidator<State>,
    ...
    fn validate_motion(&self, st1: &State, st2: &State) -> bool {
        ...
        (0..samples).any(|i| {
            ...
            !self.validate_state.validate_state(&steer_path.sample(t * steer_range.end()).expect("Steering function received out-of-range parameter."))
        }).not()
    }
}